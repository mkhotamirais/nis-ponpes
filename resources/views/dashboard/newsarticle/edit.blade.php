<x-authlayout title="Update Berita">
    ...
    <form action="{{ route('newsarticles.update', $newsarticle) }}" method="POST" enctype="multipart/form-data"
        class="">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <input type="text" class="input !w-full @error('title') !border-red-300 @enderror" name="title"
                id="title" value="{{ $newsarticle->title }}" placeholder="Judul berita">
            @error('title')
        ...
        <div class="mb-3">
            ...
            <textarea name="content" id="content" cols="30" rows="10"
                class="input @error('content') !ring-red-500 @enderror">{{ $newsarticle->content }}</textarea>
            @error('content')
                <p class="error">{{ $message }}</p>
            @enderror
        </div>
        ...
        @if ($newsarticle->banner)
            ...
            <figure class="h-40 w-64 rounded-md mb-4 overflow-hidden">
                <img id="current-image" src="{{ asset('storage/' . $newsarticle->banner) }}"
                    alt="{{ $newsarticle->title ?? 'Newsarticle Image' }}" width="400" height="400"
                    class="w-40 h-full object-cover origin-center">
            </figure>
